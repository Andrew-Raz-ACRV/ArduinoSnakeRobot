...
% Last Modified by GUIDE v2.5 08-Jun-2018 21:49:19
...
function q7_box_Callback(hObject, eventdata, handles)
% hObject    handle to q7_box (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of q7_box as text
%        str2double(get(hObject,'String')) returns contents of q7_box as a double
global q7
q7 = str2double(get(hObject,'String'));

% --- Executes during object creation, after setting all properties.
function q7_box_CreateFcn(hObject, eventdata, handles)
% hObject    handle to q7_box (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
global s;
servalue = strcat('q1=',num2str(round(1250*q1))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);
servalue = strcat('q2=',num2str(round(1250*q2))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);
servalue = strcat('q3=',num2str(round(1250*q3))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);
servalue = strcat('q4=',num2str(round((2750/pi)*q4))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);
servalue = strcat('q5=',num2str(round((2750/pi)*q5))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);
servalue = strcat('q6=',num2str(round((2750/pi)*q6))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);
servalue = strcat('q7=',num2str(round(q7))); servalue = strcat(servalue,')\n');
fprintf(s,'%s',servalue);

       %This command will send entered value to Arduino 
disp('Moving Physical Robot')
...
global target
global s;
servalue = strcat('(x',num2str(round(target(1))));
servalue = strcat(servalue,'y');
servalue = strcat(servalue,num2str(round(target(2))));
servalue = strcat(servalue,'z');
servalue = strcat(servalue,num2str(round(target(3))));
servalue = strcat(servalue,')\n'); %emulates an enter key

fprintf(s,'%s',servalue);          %This command will send entered value to Arduino 
disp('Beginning inverse kinematics')
